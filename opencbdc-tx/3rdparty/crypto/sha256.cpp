...
bool SelfTest() {
    ...
    if (TransformD64_2way) {
        unsigned char out_2way[64];
        TransformD64_2way(out_2way, data + 1);
        if (!std::equal(out_2way, out_2way + 64, result_d64)) return false;
    }

    // Test TransformD64_4way, if available.
    if (TransformD64_4way) {
        unsigned char out_4way[128];
        TransformD64_4way(out_4way, data + 1);
        if (!std::equal(out_4way, out_4way + 128, result_d64)) return false;
    }

    // Test TransformD64_8way, if available.
    if (TransformD64_8way) {
        unsigned char out_8way[256];
        TransformD64_8way(out_8way, data + 1);
        if (!std::equal(out_8way, out_8way + 256, result_d64)) return false;
    }
